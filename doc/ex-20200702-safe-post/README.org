...
*** Constraints: Basic Prolog / CLP(FD) Implementation

In this section we use Prolog and CLP(FD) to solve the example problems. We cut to the chase: gone are the types and helper functions. We set up the problem and solve it in a very few lines of code.
